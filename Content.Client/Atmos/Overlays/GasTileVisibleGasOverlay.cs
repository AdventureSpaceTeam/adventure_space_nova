...
public sealed class GasTileVisibleGasOverlay : Overlay
{
    ...
    /// Adventure edit: gas indices that should never be rendered (transparent gases)
        ...
        for (var i = 0; i < _gasCount; i++)
        {
            ...
            switch (gasPrototype.GasOverlaySprite)
            {
                case SpriteSpecifier.Rsi animated:
                    ...
                    {
                        // Adventure Edit
                        Logger.Error($"Gas overlay state '{stateId}' was not found in '{gasPrototype.GasOverlaySprite}'. Gas: {gasPrototype.ID}");
                        _frames[i] = new[] { Texture.White };

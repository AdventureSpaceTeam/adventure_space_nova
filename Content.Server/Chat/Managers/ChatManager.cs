...
using Content.Server.Discord; // Adventure
...
internal sealed partial class ChatManager : IChatManager
{
    ...
    [Dependency] private readonly DiscordLink _discordLink = default!; // Adventure
    [Dependency] private readonly ITaskManager _taskManager = default!; // Adventure
    ...
    private ulong _relayChannelId = 0; // Adventure
    private ulong _adminRelayChannelId = 0; // Adventure
    ...
    public void Initialize()
    {
        ...
        _configurationManager.OnValueChanged(ACVars.OocRelayChannelId, OnOocChannelChanged, true); // Adventure
        _configurationManager.OnValueChanged(ACVars.AhelpRelayChannelId, OnAdminChannelChanged, true); // Adventure

        _discordLink.OnMessageReceived += MessageReceived; // Adventure
    ...
    private void SendOOC(ICommonSession player, string message)
    {
        ...
        var wrappedMessage = Loc.GetString("chat-manager-send-ooc-wrap-message", ("playerName", player.Name), ("message", FormattedMessage.EscapeText(message))); // Adventure
        if (_adminManager.HasAdminFlag(player, AdminFlags.Admin))
        {
            var prefs = _preferencesManager.GetPreferences(player.UserId);
            colorOverride = prefs.AdminOOCColor;
        }
        if (_netConfigManager.GetClientCVar(player.Channel, CCVars.ShowOocPatronColor) && player.Channel.UserData.PatronTier is { } patron && PatronOocColors.TryGetValue(patron, out var patronColor)) // Adventure
        {
            wrappedMessage = Loc.GetString("chat-manager-send-ooc-patron-wrap-message", ("patronColor", patronColor), ("playerName", player.Name), ("message", FormattedMessage.EscapeText(message))); // Adventure
        }

        //TODO: player.Name color, this will need to change the structure of the MsgChatMessage
        ChatMessageToAll(ChatChannel.OOC, message, wrappedMessage, EntityUid.Invalid, hideChat: false, recordReplay: true, colorOverride: colorOverride, author: player.UserId);
        _discordLink.SendMessage($"**OOC**: `{player.Name}`: {message}", _relayChannelId); // Adventure

...
public sealed partial class AtmosphereSystem
{
        ...
        EntityUid? sparkSourceUid = null)
    {
        ...
        var hydrogen = tile.Air.GetMoles(Gas.Hydrogen); // Adventure gases

        if (tile.Hotspot.Valid)
        {
            if (soh)
            {
                if (plasma > 0.5f || tritium > 0.5f || hydrogen > 0.5f) // Adventure gases
                {
        ...
        if (exposedTemperature > Atmospherics.PlasmaMinimumBurnTemperature && (plasma > 0.5f || tritium > 0.5f || hydrogen > 0.5f)) // Adventure gases
        {

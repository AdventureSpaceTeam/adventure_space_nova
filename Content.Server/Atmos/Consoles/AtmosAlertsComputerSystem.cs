...
using Content.Shared.PowerCell.Components; // Adventure-edit
...
public sealed class AtmosAlertsComputerSystem : SharedAtmosAlertsComputerSystem
{
    ...
    private void Beep(EntityUid ent, AtmosAlertsComputerComponent entConsole, AtmosAlarmType highestAlert)
    {
        if (entConsole.NextBeep >= _gameTiming.CurTime || highestAlert != AtmosAlarmType.Danger ||
            entConsole.BeepSound == null || !entConsole.DoAtmosAlert) // Adventure-edit
            return;
        ...
        TransformComponent xform)
    {
        ...
        _userInterfaceSystem.SetUiState(uid, AtmosAlertsComputerUiKey.Key,
            new AtmosAlertsComputerBoundInterfaceState(airAlarmStateData, fireAlarmStateData, focusAlarmData, component.DoAtmosAlert));
    }

using System.Linq;
using Content.Server.AdventurePrivate._Alteros.DarkForces.Narsi.Buildings.Altar.Rituals.Polymorth;
using Content.Shared.AdventureSpace.DarkForces.Narsi.Roles;
using Content.Shared.Damage;
using Content.Shared.Mobs.Systems;
using Robust.Shared.Timing;

namespace Content.Server.AdventurePrivate._Alteros.DarkForces.Narsi.Buildings.Pilon;
...
public sealed class NarsiCultPilonSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (pilonQuery.MoveNext(out var uid, out var pilon))
        {
            ...
            var cultistsPolymorphNear =
                _entityLookupSystem.GetEntitiesInRange<NarsiPolymorphComponent>(pilonCoords, 6.0f);
            if (cultistsPolymorphNear.Any())
                RegenPolymorph(cultistsPolymorphNear, pilon);
        }
    }

    private void RegenPolymorph(HashSet<Entity<NarsiPolymorphComponent>> cultists,
        NarsiCultPilonComponent pilonComponent)
    {

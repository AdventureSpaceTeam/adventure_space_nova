...
public sealed partial class NarsiRuneSystem
{
    ...
    private void OnSummonDoAfterEvent(EntityUid uid, NarsiCultistComponent component, SummonNarsiRuneDoAfterEvent args)
    {
        ...
        args.Handled = true;
        _audioSystem.PlayEntity(RuneSound,
    ...
    private void TryToSummond(EntityUid rune, EntityUid user)
    {
        ...
        var state = new SecretListingInitDataState("NarsiSummonRune",
            data,
            "Выберите культиста",
            "Если культист связан И его кто-то держит, призыв не получится",
            true,
            GetNetEntity(user));
        _ui.SetUiState(rune, SecretListingKey.Key, state);
    }

    private List<ListingData> GetCultistsListing(EntityUid user,
        EntityQueryEnumerator<NarsiCultistComponent> enumerator)
    {

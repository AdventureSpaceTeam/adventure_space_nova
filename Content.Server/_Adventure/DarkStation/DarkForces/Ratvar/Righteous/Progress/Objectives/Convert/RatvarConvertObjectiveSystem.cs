...
public sealed class RatvarConvertObjectiveSystem : EntitySystem
{
    ...
    private void OnGetProgress(EntityUid uid,
        RatvarConvertObjectiveComponent component,
        ref ObjectiveGetProgressEvent args)
    {
        var righteouses = EntityQuery<RatvarRighteousComponent>();
        var maradeurs = EntityQuery<RatvarMarauderShellComponent>()
            .Where(marouder => _mindSystem.TryGetMind(marouder.Owner, out _, out _));

...
namespace Content.Server.Administration.Systems
{
    ...
    public sealed partial class BwoinkSystem : SharedBwoinkSystem
    {
        ...
        public override void Initialize()
        {
            ...
            Subs.CVar(_config, ACVars.AhelpRelayChannelId, OnChannelIdChanged, true);
            Subs.CVar(_config, ACVars.AhelpRelayShowDiscord, OnShowDiscordChanged, true);
        ...
        private async void OnReceiveNewRelay(CommandReceivedEventArgs commandArgs)
        {
            ...
            {
                await commandArgs.Message.Channel.SendMessageAsync("**Внимание**: Уже есть ветка с ахелпом этого игрока.");
            ...
            if (lookup is null)
            {
                ...
                await commandArgs.Message.Channel.SendMessageAsync("**Внимание**: Не смогли найти дискорд ID игрока.");
                return;
            }
            ...
            if (channel is null)
            {
                await commandArgs.Message.Channel.SendMessageAsync("**Внимание**: Не смогли создать ветку с ахелпом.");
                return;
            }
            ...
            // Notify admins
            await channel.SendMessageAsync($"**Важно:** Ветка создана. Последующие сообщения будут переданы игроку. <@{commandArgs.Message.Author.Id}>");
            await commandArgs.Message.Channel.SendMessageAsync($"<#{relay.channel.Id}>");
        }
        ...
        private void OnDiscordMessageReceived(SocketMessage arg)
        {
            ...
            foreach (var messages in _relayMessages)
            {
                ...
                if (!_playerManager.TryGetSessionById(messages.Key, out var session))
                {
                    ...
                    arg.Channel.SendMessageAsync("**Внимание**: Не удалось найти сессию для игрока. Он может быть не в сети.");
                    continue;
                }
                ...
                if (arg.Content.StartsWith(_discord.BotPrefix))
                {
                    ...
                    switch (contentWithoutPrefix)
                    {
                        case "status":
                            ...
                            break;
                        default:
                            arg.Channel.SendMessageAsync("**Внимание**: Неизвестная команда.");
                            break;
                    }
                    continue;
                }
                ...
                var content = "[color=yellow] (д) " + arg.Author.Username + "[/color]: " + arg.Content;
            ...
            if (_relayMessages.TryGetValue(e.Session.UserId, out var relay))
            {
                switch (e.NewStatus)
                {
                    case SessionStatus.Connected:
                        relay.channel.SendMessageAsync($"**Внимание**: {e.Session.Name} переподключился. Последующие сообщения будут доставлены игроку.");
                    ...
                    case SessionStatus.Disconnected:
                        relay.channel.SendMessageAsync($"**Внимание**: {e.Session.Name} отключился. Последующие сообщения не будут доставлены.");
                        foreach (var admin in GetTargetAdmins())
                        {
        ...
        private void OnGameRunLevelChanged(GameRunLevelChangedEvent args)
        {
            foreach (var relayMessage in _relayMessages)
            {
                ...
                {
                    GameRunLevel.PreRoundLobby => "\n\n:arrow_forward: _**Началось пред-раундовое лобби**_\n",
                    GameRunLevel.InRound => "\n\n:arrow_forward: _**Начался раунд**_\n",
                    GameRunLevel.PostRound => "\n\n:stop_button: _**Раунд закончился**_\n",
            ...
            foreach (var relayMessage in _relayMessages)
            {
                relayMessage.Value.channel.SendMessageAsync("**Внимание**: Реле закрыто. Сервер перезапускается либо раунд закончился. Последующие сообщения не будут доставлены.");
            }
        ...
        public override void Shutdown()
        {
            ...
            {
                message.Value.channel.SendMessageAsync("**Внимание**: Сервер выключается.", false, null, null, AllowedMentions.None);
            }
        ...
        private async Task<RestThreadChannel?> RequestNewRelay(string title, NetUserId targetPlayer)
        {
            ...
            {
                await relay.SendMessageAsync($"Используйте `{_discord.BotPrefix}status` что бы обновить эмбед выше. `-` в начале сообщения приведет к тому, что оно не будет передано игроку.");
                ...
                {
                    await relay.SendMessageAsync($"**Инфо**: Перейдите в предыдущую ветку: <#{oldRelay.channel.Id}>");
        ...
        private EmbedBuilder GenerateEmbed(ICommonSession session)
        {
            ...
            var gameRules = string.Join(", ", _gameTicker.GetActiveGameRules()
                ...
                .Select(meta => meta.EntityPrototype?.ID ?? meta.EntityPrototype?.Name ?? "Неизвестно")
                .ToList());
            ...
            var antagStatus = "Неизвестно";
            if (playerInfo != null)
            {
                antagStatus = playerInfo.Antag ? "Антаг" : "Не антаг";
            }
            ...
            var embed = new EmbedBuilder()
            {
                ...
                Fields = new List<EmbedFieldBuilder>()
                {
                    ...
                    {
                        Name = "Текущий режим",
                        Value = _gameTicker.Preset?.ID ?? "Неизвестно"
                    },
                    new()
                    {
                        Name = "Активные геймрулы",
                        Value = gameRules
                    },
                    new()
                    {
                    ...
                    new()
                    {
                        Name = "Статус антагониста",
                        Value = antagStatus
                    },
                    new()
                    {
                        Name = "Карта",
                        Value = _mapManager.GetSelectedMap()?.MapName ?? "Неизвестно"
                    },
                    new()
                    {
                        Name = "Профессия на ID карте",
                        Value = job
                    },
                    new()
                    {
